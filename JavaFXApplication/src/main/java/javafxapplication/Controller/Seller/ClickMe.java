 ...
 * @author Akim
 */
public class ClickMe implements Initializable {
    ...
    ShopProxy shopProxy = new ShopProxy();
     ...
     public void clickMe(ActionEvent event) {
       ...
       TreeItem<String> root = new TreeItem("Dukondordun tizmesi");
       ...
       for(Shop shop : shops){
           ...
           for(Seller  seller : sellers){
                ...
                System.out.println(seller.getShopname().toString()+ "  "+shop.getName().toString());
                if(seller.getShopname().toString() == shop.getName().toString())
                    node.getChildren().add(nod);
           }
       }
       
       /*for(Seller s : sellers){
           TreeItem<String> node = new TreeItem(s.getFullName());
           root.getChildren().add(node);
